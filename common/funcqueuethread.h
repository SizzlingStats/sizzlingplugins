		...
		while(!m_bExit)
		{
			...
			while(failed)
			{
				...
				while(bRunLoop)
				{
					...
					CFunctor *pCurrentFunc = const_cast<CFunctor*>(m_Waiting.Element(0));
					SOPDLog("[QUEUETHREAD] m_listMutex.Unlock()\n");
					m_listMutex.Unlock();
					...
					(*pCurrentFunc)();
					failed = false; // can't get the return value of a functor like this
...
private:
	...
	CUtlVector<const CFunctor*> m_Waiting;
	bool m_bExit;
};
